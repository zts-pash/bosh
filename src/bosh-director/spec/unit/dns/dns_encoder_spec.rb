...
module Bosh::Director
  describe DnsEncoder do
    ...
    describe '#encode_link' do
      let(:short_dns_enabled) { false } # links can *only* be encoded with short dns; disabled here as a reminder
      ...
      it 'accepts additional query criteria' do
        # TODO ordering of network and status might be flaky
        expect(subject.encode_link(link_def, status: 'healthy', default_network: default_network, root_domain: 'bosh')).to eq("q-n#{default_network_id}s3.q-g12.bosh")
      end
    end
